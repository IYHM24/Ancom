    ...
    <header className={` ${colorMode == "light" ? abierto ? "bg-white" : "bg-navbar-light" : abierto ? "bg-black" : "bg-navbar-dark"} shadow-sm !sticky !top-0 !z-50 !w-full !border-gray-200 dark:!border-gray-800 backdrop-blur`}>
      <div className='!container !mx-auto flex justify-between max-md:!p-5'>
        <div className='flex gap-5'>
          ...
          {/* logo */}
          <Link to="/" className='flex items-center' onClick={() => { handleClick(),setAbierto(false) }}>
          ...
          <div className='flex justify-center items-center gap-5 max-md:hidden'>
            {mapRoutes.map((item, indice) => (
              <Link onClick={handleClick} className='!text-gray-700 dark:!text-gray-300 align-bottom w-max h-max !font-light' key={"item-nav-" + indice} to={item.path.path}> {item.nombre} </Link>
            ))}
          </div>
      ...
      <div className={` ${!abierto && "hidden"} md:hidden top-20 fixed z-10 w-full !backdrop-blur h-[200vh] ${colorMode == "light" ? "bg-navbar-light" : "bg-navbar-dark"}`}>
        <div className={` ${colorMode == "light" ? "bg-white" : "bg-black"} flex gap-2 flex-col`}>
          <div className='flex flex-col !p-2 gap-2'>
            {mapRoutes.map((item, indice) => (
              <Link
                ...
                key={"item-nav-" + indice}
                to={item.path.path}
                onClick={() => { handleClick(), setAbierto(false) }}
              >
                {item.nombre}
              </Link>
            ))}
          ...
          <div className={`border-t-2 ${colorMode == "light" ? "!border-black" : "!border-white "} !px-2 !py-5 flex flex-col gap-2`}>
            <Link onClick={handleClick} target="_blank" to={"https://github.com/IYHM24"}> <Button className={`w-full !bg-transparent ${colorMode == "light" ? "!border-black" : "!border-white "} dark:hover:!bg-gray-900 dark:!bg-opacity-50`}> <LuGithub stroke={colorMode == 'light' ? 'black' : 'white'} /> </Button> </Link>
            <Link onClick={handleClick} target="_blank" to={"https://api.whatsapp.com/send?phone=3506930989"}><Button className='w-full !font-bold !bg-black dark:!bg-white dark:!text-black'> Contactarnos </Button></Link>
          </div>
        </div>
